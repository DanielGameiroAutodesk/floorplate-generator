import type { PolygonXY } from "src/integrations/building-systems-common/buildingMigrations/pureMigrationFunctions/geometry/geometry"
import {
  areaOfPolygon,
  isPointInsidePolygon,
  isPolygonClockwise,
} from "src/integrations/building-systems-common/buildingMigrations/pureMigrationFunctions/geometry/geometry"
...
export function findPolygonsWithHolesInGraph(graph: Graph) {
  ...
  subGraphUnitsAndGroup.forEach(({ polygons, groupId }) => {
    ...
    const subUnits = polygons.map((polygon): GroupedPolygonWithHoles => {
      const holes = findHolesInPolygon(groupId, polygon, childGroups, subGraphUnitsAndGroup).map((hole) => {
        if (isPolygonClockwise(hole)) return hole
        return [...hole].reverse()
      })
      return {
        polygon,
