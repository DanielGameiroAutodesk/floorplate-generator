...
import { generateUUID as uuidv4 } from "./uuidTemp"
...
function getNormalizedVectorFromPointToPoint(p1: number[], p2: number[]) {
...
export function cleanBufferedPolygon(polygon: Polygon, threshold: number) {
  if (polygon.length <= 4) {
    return polygon
  }

  let cleanedPolygon: number[][] = []
