import {
  ...
  normalizedVectorFromPoints,
  normalizeVector,
  pointPointDistance,
  pointToLineDistance,
  pullBackMidPoint,
} from "./geometry"
import { generateId } from "./uuidTemp"
...
function getNormalizedVectorFromPointToPoint(p1: Point, p2: Point) {
...
function pointsAreEqual(p1: number[], p2: number[], precision: number) {
  if (distance(p1, p2) < precision) return true
  return false
}

function removeLastPointInPolygonIfEqualsFirst(polygon: Polygon) {
...
export function simplifyOpenPolygon(polygon: Polygon, epsilon: number): Polygon {
  let maxDist = 0
  let index = 0
  ...
  if (maxDist > epsilon) {
    const firstHalf = simplifyOpenPolygon(polygon.slice(0, index + 1), epsilon)
    const secondHalf = simplifyOpenPolygon(polygon.slice(index), epsilon)
...
function getPolygonEdgeVectors(polygon: Polygon) {
  let edgeVectors = []
...
function getNormalizedPolygonEdgeVectors(polygon: Polygon) {
...
export function preprocessPolygonWithSimplification(polygon: Polygon) {
  const copiedPolygon = copyPolygon(polygon)
  const processedPolygon = removeLastPointInPolygonIfEqualsFirst(getCCWPolygon(copiedPolygon))
...
export function polygonToOffsetLinesWithConstantOffset(polygon: Polygon, offset: number) {
...
export function polygonToOffsetLinesWithVaryingOffset(polygon: Polygon, offsetList: number[]) {
  ...
  for (let i = 0; i < offsetList.length; i++) {
    if (offsetList[i] > 0) {
      ...
      const offsetPoints: Point[] = [
        addVectorToPoint(points[0], offsetVector),
        addVectorToPoint(points[1], offsetVector),
      ]
...
export function polygonsToGraph(polygons: Polygon[], edgeWidth: number) {
  let vertices = {}
  let edges = {}
  polygons.forEach((poly) => {
    const { vertices: verticesTemp, edges: edgesTemp } = polygonToGraph(poly, edgeWidth)
...
export function polygonToGraphAndSelectedEdgeIDs(polygon: Polygon, edgeIndexes: number[], edgeWidth: number = 0) {
  const width = edgeWidth ? edgeWidth : 0
  const n = polygon.length
  const verticesIDs = polygon.map(() => generateId())
  const vertices = polygon.reduce<{ [id: string]: { id: string; x: number; y: number } }>((acc, curr, i) => {
    const newVertex = { id: verticesIDs[i], x: curr[0], y: curr[1] }
    acc[newVertex.id] = newVertex
    return acc
  }, {})
  ...
  const edges = edgesValues.reduce<{ [id: string]: (typeof edgesValues)[0] }>((acc, curr) => {
    acc[curr.id] = curr
    return acc
  }, {})
...
export function cleanBufferedPolygon(polygon: Polygon, threshold: number): Polygon {
  if (polygon.length <= 4) {
    return polygon
  }

  let cleanedPolygon: Point[] = []
  const _polygon = polygon.slice(1)
  const n = _polygon.length

  try {
    for (let i = 0; i < n; i++) {
      const p0: Point = i === 0 ? _polygon[_polygon.length - 1] : cleanedPolygon[cleanedPolygon.length - 1]
      const p1 = _polygon[i]
