...
export function cleanPolygon(polygon: Polygon) {
  const cleanedPolygon = getCCWPolygon(polygon)
    .map((p) => p.map((v) => parseFloat(v.toFixed(3))))
    .filter((p, i, l) => p[0] !== l[(i + 1) % l.length][0] || p[1] !== l[(i + 1) % l.length][1]) as Polygon
...
export function addVectorToPoint(point: number[], vector: number[]): Point {
...
export function pointToLineDistance(point: number[], line: number[][]) {
