...
function RoadTrafficProperties({ selectedRoadNodes }: { selectedRoadNodes: ChildNodeContainer[] }) {
  ...
  const selectedTrafficData = useMemo(() => {
    return selectedRoadNodes.map((elm) => elm.element.properties?.trafficData as RoadTrafficData)
  }, [selectedRoadNodes])
  ...
  const addTrafficData = useCallback(() => {
    ...
    const { child, element, elementContainer } = selectedRoadNodes[0]
  ...
  }, [contextRoot, selectedRoadNodes])
  const removeTrafficData = useCallback(() => {
    const updatedElementContainers = selectedRoadNodes.map((node) => {
  ...
  const updateTrafficDataProperty = useCallback(
    (description: string, updatedProperties: Partial<RoadTrafficData>) => {
      ...
      selectedRoadNodes.forEach(({ element, child, elementContainer }) => {
        elementState.edit(({ updateElement }) => {
      ...
      {!isHidden && (
        <>
          {!allRequiredInputsFilled && (
            <div className={styles.Alerts}>{t(($) => $.transportation.trafficData.roadRequiredInputsMessage)}</div>
          )}
...
export default RoadTrafficProperties
